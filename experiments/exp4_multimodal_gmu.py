# -*- coding: utf-8 -*-
"""
experiments/exp4_multimodal_gmu.py
===================================
Eksperimen 4: Multimodal dengan Gated Multimodal Unit (GMU) — METODE UTAMA.
...
def run_exp4_multimodal_gmu(config_path=None, override=None):
    ...
    result_main_path = os.path.join(config['paths']['results'], 'multimodal_gmu')
    os.makedirs(result_main_path, exist_ok=True)

    cv_base_path = os.path.join(config['paths']['results'], 'robustness_cv', 'multimodal_gmu')
    os.makedirs(cv_base_path, exist_ok=True)

    cache_dir = os.path.join(config['paths']['embeddings_cache'], 'gmu')
    ...
    print(f"\nFusion Config: {fusion_config['fusion_type']} (Metode Utama)")
    print(f"  Fusion Dim     : {fusion_config['fusion_dim']}")
    print(f"  N Folds        : {n_folds} (GroupKFold - Actor-Based)")
    print(f"  Internal Split : YES (aktor sama untuk early stopping)")
    ...
    temp_fusion_model = MultimodalFusionModel(
        num_classes=8,
        fusion_type='gmu',
    ...
    optimizer = torch.optim.AdamW(
        temp_fusion_model.parameters(),
        lr=fusion_config['fusion_lr'],
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(gkf.split(audio_embeds_all, labels_all, groups=actor_ids_all_cv)):
        ...
        fold_sched = torch.optim.lr_scheduler.OneCycleLR(
            fold_opt,
            max_lr=0.01,
            epochs=fusion_config['n_epochs'] // 2,
        ...
        if fold_result['best_prec1'] > best_fold_acc:
            ...
            best_fold_model_path = fold_result['result_path'] + '/multimodal_gmu_best.pth'
    ...
    if best_fold_model_path and os.path.exists(best_fold_model_path):
        ...
        main_model_path = os.path.join(result_main_path, 'multimodal_gmu_best.pth')
        shutil.copyfile(best_fold_model_path, main_model_path)
        print(f"✅ Best model copied to {main_model_path} for exp7 comparison.")

        try:
            roofline_analysis(best_model, 'Multimodal GMU (Metode Utama)', device, is_multimodal=True)
    ...
    print("\n" + "="*60)
    print("  5-FOLD ACTOR-BASED CV RESULTS (GMU)")
    ...
    print(f"\nEksperimen 4 (GMU) selesai")
    return best_model, val_metrics if 'val_metrics' in locals() else None
...
def main():
    ...
    run_exp4_multimodal_gmu(args.config)
