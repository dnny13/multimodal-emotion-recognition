# -*- coding: utf-8 -*-
"""
experiments/exp3_multimodal_concat.py
=====================================
Eksperimen 3: Multimodal dengan Concatenation Fusion (Baseline Fusion).
...
def run_exp3_multimodal_concat(config_path=None, override=None):
    ...
    result_main_path = os.path.join(config['paths']['results'], 'multimodal_concat')
    os.makedirs(result_main_path, exist_ok=True)

    cv_base_path = os.path.join(config['paths']['results'], 'robustness_cv', 'multimodal_concat')
    os.makedirs(cv_base_path, exist_ok=True)

    cache_dir = os.path.join(config['paths']['embeddings_cache'], 'concat')
    ...
    print(f"\n📐 Fusion Config: {fusion_config['fusion_type']} (Baseline)")
    ...
    train_dataset = RavdessMultimodal(annotation_path, audio_root, visual_root, 'training',
                                      transform_audio_train, transform_visual_train, num_frames)
    val_dataset = RavdessMultimodal(annotation_path, audio_root, visual_root, 'validation',
                                    transform_audio_val, transform_visual_val, num_frames)
    test_dataset = RavdessMultimodal(annotation_path, audio_root, visual_root, 'testing',
                                     transform_audio_val, transform_visual_val, num_frames)

    # ---- Internal Split (Actor 1-16) ----
    ...
    scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, max_lr=0.01, epochs=fusion_config['n_epochs'],
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(gkf.split(audio_embeds_all, labels_all, groups=actor_ids_all_cv)):
        train_embeds = {'audio': audio_embeds_all[train_idx], 'visual': visual_embeds_all[train_idx], 'labels': labels_all[train_idx]}
        val_embeds = {'audio': audio_embeds_all[val_idx], 'visual': visual_embeds_all[val_idx], 'labels': labels_all[val_idx]}
        ...
        fold_sched = torch.optim.lr_scheduler.OneCycleLR(fold_opt, max_lr=0.01, epochs=fusion_config['n_epochs']//2,
                                                         steps_per_epoch=steps_per_epoch_fold, pct_start=0.3, div_factor=10, final_div_factor=100)
        fold_result = train_multimodal_fold(
            fold_model, train_embeds, val_embeds, criterion, fold_opt, fold_sched,
            device, fold_idx, fusion_config,
            cross_val_loader=cross_actor_val_loader
        )
        ...
        if fold_result['best_prec1'] > best_fold_acc:
            ...
            best_fold_model_path = fold_result['result_path'] + '/multimodal_concat_best.pth'
    ...
    if best_fold_model_path and os.path.exists(best_fold_model_path):
        ...
        _, val_acc, val_metrics = validate_epoch_with_cache(-1, test_loader_emb, best_model, nn.CrossEntropyLoss(), device, None)
        ...
        main_model_path = os.path.join(result_main_path, 'multimodal_concat_best.pth')
        shutil.copyfile(best_fold_model_path, main_model_path)
        print(f"✅ Best model copied to {main_model_path} for exp7 comparison.")

        roofline_analysis(best_model, 'Multimodal Concatenation', device, is_multimodal=True)

    # ---- CV Summary ----
    accuracies = [r['best_prec1'] for r in fold_results]
    print("\n" + "="*60)
    print("  5-FOLD ACTOR-BASED CV RESULTS (Concat)")
    for i, acc in enumerate(accuracies): print(f"  Fold {i+1}: {acc:.2f}%")
    print(f"  Mean: {np.mean(accuracies):.2f}% (±{np.std(accuracies):.2f})")
    print("="*60)
    cv_summary_path = os.path.join(cv_base_path, 'cv_summary.csv')
    pd.DataFrame({'fold': list(range(1, len(accuracies)+1)), 'best_prec1': accuracies,
                  'f1_macro': [r.get('best_f1', 0) for r in fold_results]}).to_csv(cv_summary_path, index=False)
...
def main():
    ...
    run_exp3_multimodal_concat(args.config)
